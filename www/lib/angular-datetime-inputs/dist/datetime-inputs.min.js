...
angular.module("g1b.datetime-inputs").directive("dateInput", ["$document", "$timeout", function(e, a) {
    return {
        ...
        replace: !0,
        template: "<div class=\"datetime-input date\"><div ng-click=\"toggleEditPopover()\" class=\"display\" ng-class=\"{'active': !!selected, 'warning': !!warning, '{{cssClass}}': !!cssClass}\">" +
        "<div class=\"date\" ng-bind=\"date.format(dateFormat || 'YYYY/MM/DD')\"></div>" +
        "<div ng-if=\"!date\" class=\"placeholder\" ng-bind=\"placeholder\"></div></div>" +
        "<div class=\"edit-popover\" ng-show=\"!!selected\">" +
        "<div class=\"header\" ng-bind=\"selected.format('YYYY/MM/DD')\"></div>" +
        "<div class=\"calendar\"><div class=\"calendar-header\" scroll-up=\"calendar.add(1, 'months')\" scroll-down=\"calendar.subtract(1, 'months')\">" +
        ...
        "<div class=\"arrow arrow-right\" ng-click=\"calendar.add(1, 'months')\"></div></div>" +
        "<div class=\"calendar-body\"><div class=\"weekdays\"><span class=\"weekday\" ng-repeat=\"weekday in 'weeeeek' track by $index\" ng-bind=\"calendar.clone().startOf('week').add($index, 'days').format('ddd')\"></span>" +
        "</div>" +
        ...
        "<span class=\"date\" ng-repeat=\"date in 'weeeeek' track by $index\" ng-class=\"{ 'current': calendar.clone().startOf('month').add($parent.$index, 'weeks').weekday($index).startOf('day').isSame(current.clone().startOf('day')), 'active': calendar.clone().startOf('month').add($parent.$index, 'weeks').weekday($index).startOf('day').isSame(selected.clone().startOf('day')), 'inactive': calendar.clone().startOf('month').add($parent.$index, 'weeks').weekday($index).month() !== calendar.month() }\" ng-click=\"update(selected.clone().year(calendar.clone().startOf('month').add($parent.$index, 'weeks').weekday($index).year()).month(calendar.clone().startOf('month').add($parent.$index, 'weeks').weekday($index).month()).date(calendar.clone().startOf('month').add($parent.$index, 'weeks').weekday($index).date()), true)\" ng-bind=\"calendar.clone().startOf('month').add($parent.$index, 'weeks').weekday($index).date()\"></span></div></div></div>" +
...
angular.module("g1b.datetime-inputs").directive("timeInput", ["$document", "$timeout", function(e, t) {
    return {
        ...
        replace: !0,
        template: '<div class="datetime-input time">' +
        '<div ng-click="toggleEditPopover()" class="display" ng-class="{\'active\': !!selected, \'warning\': !!warning, \'{{cssClass}}\': !!cssClass}">' +
        '<div class="time" ng-bind="time.format(timeFormat || \'HH : mm : ss\')"></div>' +
        '<div ng-if="!time" class="placeholder" ng-bind="placeholder"></div>' +
        '</div>' +
        '<div class="edit-popover" ng-show="!!selected">' +
        '<div class="timer">' +
        '<div class="timer-hours" scroll-up="update(selected.clone().add(hourStep || 1, \'hours\'))" scroll-down="update(selected.clone().subtract(hourStep || 1, \'hours\'))">' +
        '<div class="arrow arrow-up" ng-click="update(selected.clone().add(hourStep || 1, \'hours\'))"></div>' +
        '<span ng-bind="selected.format(\'HH\')"></span><div class="arrow arrow-down" ng-click="update(selected.clone().subtract(hourStep || 1, \'hours\'))"></div></div>' +
        '<div class="timer-divider">:</div>' +
        '<div class="timer-minutes" scroll-up="update(selected.clone().add(minuteStep || 1, \'minutes\'))" scroll-down="update(selected.clone().subtract(minuteStep || 1, \'minutes\'))">' +
        '<div class="arrow arrow-up" ng-click="update(selected.clone().add(minuteStep || 1, \'minutes\'))"></div><span ng-bind="selected.format(\'mm\')"></span>' +
        '<div class="arrow arrow-down" ng-click="update(selected.clone().subtract(minuteStep || 1, \'minutes\'))"></div></div>' +
        '<div class="timer-divider">:</div>' +
        '<div class="timer-seconds" scroll-up="update(selected.clone().add(secondStep || 1, \'seconds\'))" scroll-down="update(selected.clone().subtract(secondStep || 1, \'seconds\'))">' +
        '<div class="arrow arrow-up" ng-click="update(selected.clone().add(secondStep || 1, \'seconds\'))"></div>' +
        '<span ng-bind="selected.format(\'ss\')"></span>' +
        '<div class="arrow arrow-down" ng-click="update(selected.clone().subtract(secondStep || 1, \'seconds\'))"></div></div></div><div class="clear-button" ng-if="!!time && allowClear"><div ng-click="update()" ng-bind="clearText || \'Clear\'"></div></div></div></div>',
        compile: function() {
            return {
                pre: function() {},
                post: function(s, d) {
